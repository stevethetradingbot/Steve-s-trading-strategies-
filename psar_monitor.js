// Live Trading Monitor - Parabolic SAR Strategy
...
const AF = 0.07; // Acceleration factor (optimized!)
...
function fetchData() {
    return new Promise((resolve, reject) => {
        https.get(`https://min-api.cryptocompare.com/data/v2/histohour?fsym=${COIN}&tsym=USD&limit=100`, res => {
            let data = '';
...
try {
    const saved = JSON.parse(fs.readFileSync('paper_state.json', 'utf8'));
...
async function main() {
    ...
    // Save state
    fs.writeFileSync('paper_state.json', JSON.stringify(state));
}
