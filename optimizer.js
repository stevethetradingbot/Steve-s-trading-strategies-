/**
 * OPTIMIZER - Find the best SL/TP combo
 */
...
const COMBOS = [
    { sl: 3, tp: 9, name: "3%/9%" },
    { sl: 5, tp: 10, name: "5%/10%" },
    { sl: 5, tp: 15, name: "5%/15%" },
    { sl: 5, tp: 20, name: "5%/20%" },
    { sl: 7, tp: 14, name: "7%/14%" },
...
async function main() {
    ...
    for (const coin of COINS) {
        ...
        for (const combo of COMBOS) {
            ...
            console.log(`   ${combo.name}: ${emoji} ${result.return.toFixed(2)}% | ${result.wins}W/${result.losses}L`);
        }
    }
    ...
    console.log(`\n╔═══════════════════════════════════════════════════════╗`);
    console.log(`║           TOP 5 SETTINGS                            ║`);
    console.log(`╠═══════════════════════════════════════════════════════╣`);
    
    for (let i = 0; i < 5; i++) {
        const r = allResults[i];
        console.log(`║ ${i+1}. ${r.coin} ${r.combo.padEnd(10)} → ${r.return > 0 ? '+' : ''}${r.return.toFixed(2)}%`.padEnd(48) + `║`);
    }
    console.log(`╚═══════════════════════════════════════════════════════╝`);
    
    const best = allResults[0];
    console.log(`\n🏆 RECOMMENDED: ${best.coin} with ${best.combo}`);
    console.log(`   Stop Loss: ${best.sl}% | Take Profit: ${best.tp}%`);
    console.log(`   Expected Return: +${best.return.toFixed(2)}%`);
}
